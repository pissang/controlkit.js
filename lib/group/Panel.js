...
Panel.prototype._constrainHeight = function () {
    ...
    if (heightDiff < -2.0) {    // Constraint layout will be recursive infinitely if heightDiff is -1
        heightSum = panelHeight + heightDiff;

        if (!hasScrollWrap) {
            this._addScrollWrap(heightSum);
            // Avoid recursive when change from scroll to no scroll.
            // if (heightDiff !== this._heightDiff) {
